...
export class DeletemodalComponent {
  ...
  validatePassword(): void {
    if (
      ...
      Md5.hashStr(this.password) !== '84892b91ef3bf9d216bbc6e88d74a77c'
    ) {
  ...
  deleteReport(key: number): void {
    this.http
      .delete<Report[]>(this.apiURL + 'reports/documents/' + key + '/')
      .subscribe({
        next: () => {
          location.reload();
        },
  ...
  decrementLocation(): void {
    ...
    if (this.locationTemp.count === 1) {
      ...
      this.http
        .put<Location[]>(
          this.apiURL + 'locations/documents/' + this.locationTemp.key + '/', obj)
        .subscribe({
          next: (data) => {
            // console.log(data);
          },
  ...
  filterLocation(locations: Location[], locationName: string): void {
    (Object.keys(locations) as (keyof typeof locations)[]).forEach(
      (key): any => {
  ...
  deleteLocation(key: number): void {
    this.http
      .delete<Location[]>(this.apiURL + 'locations/documents/' + key + '/')
      .subscribe({
        next: () => {
          // location.reload();
        },
